...
export default function CustomerDetailClient({ customer: init, sites: initSites, transformers: initTx, canEdit }: Props) {
  ...
  function cset(k: string, v: string) { setCustForm(f => ({ ...f, [k]: v })) }

  async function saveCust() {
    setCustSaving(true)
    setCustError('')
    const { error } = await updateCustomer(customer.id, {
      name: custForm.name, type: custForm.type,
      contact_person: custForm.contact_person, phone: custForm.phone,
      email: custForm.email || null, whatsapp_number: custForm.whatsapp_number || null,
    })
    setCustSaving(false)
    if (error) { setCustError(error); return }
    setCustomer(c => ({
      ...c, name: custForm.name, type: custForm.type as Customer['type'],
      contact_person: custForm.contact_person, phone: custForm.phone,
      email: custForm.email || null, whatsapp_number: custForm.whatsapp_number || null,
      designation: custForm.designation || null,
      sap_customer_code: custForm.sap_customer_code || null,
    }))
  ...
  function startEditTx(t: Transformer) {
    setTxEditing(t.id)
    setTxForm({
  ...
  async function handleAdd() {
    ...
    // Reload page to get fresh data with new IDs
    ...
    <>
      ...
      <div style={{ background: '#fff', borderRadius: 10, border: '1px solid var(--gm)', padding: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          ...
          {canEdit && !custEditing && (
            <button onClick={() => { setCustEditing(true); setCustError('') }} style={iconBtn()}>
              <EditIcon />
            </button>
          )}
        </div>

        {custError && <div style={{ background: '#FEE2E2', color: '#DC2626', borderRadius: 7, padding: '8px 12px', fontSize: 12, marginBottom: 12 }}>{custError}</div>}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                ...
                <input required style={fi} value={custForm.name} onChange={e => cset('name', e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 10, color: 'var(--txm)', display: 'block', marginBottom: 3 }}>Customer type</label>
                <select style={sel} value={custForm.type} onChange={e => cset('type', e.target.value)}>
              ...
              <div>
                ...
                <input required style={fi} value={custForm.contact_person} onChange={e => cset('contact_person', e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 10, color: 'var(--txm)', display: 'block', marginBottom: 3 }}>Designation</label>
                <input style={fi} value={custForm.designation} onChange={e => cset('designation', e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 10, color: 'var(--txm)', display: 'block', marginBottom: 3 }}>Phone *</label>
                <input required style={fi} value={custForm.phone} onChange={e => cset('phone', e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 10, color: 'var(--txm)', display: 'block', marginBottom: 3 }}>Email</label>
                <input type="email" style={fi} value={custForm.email} onChange={e => cset('email', e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 10, color: 'var(--txm)', display: 'block', marginBottom: 3 }}>WhatsApp</label>
                <input style={fi} value={custForm.whatsapp_number} onChange={e => cset('whatsapp_number', e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 10, color: 'var(--txm)', display: 'block', marginBottom: 3 }}>SAP code</label>
                <input style={fi} value={custForm.sap_customer_code} onChange={e => cset('sap_customer_code', e.target.value)} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => { setCustEditing(false); setCustError('') }} disabled={custSaving}
              ...
              <button onClick={saveCust} disabled={custSaving}
                style={{ padding: '7px 14px', borderRadius: 7, border: 'none', background: 'var(--m)', color: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 500, fontFamily: 'Poppins,sans-serif', opacity: custSaving ? .7 : 1 }}>
      ...
      {/* ── Transformer table ─────────────────────────────────────────────── */}
      <div style={{ background: '#fff', borderRadius: 10, border: '1px solid var(--gm)', overflow: 'hidden', marginTop: 14 }}>
                      ...
                      <td style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}>
                        {isEditing ? (
                          ...
                          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                            ...
                            <button onClick={() => handleDeleteTx(t.id)} disabled={txDeleting === t.id}
                          ...
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button onClick={() => startEditTx(t)} style={iconBtn()} title="Edit"><EditIcon /></button>
