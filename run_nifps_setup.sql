-- NIFPS Setup — paste and run in Supabase SQL Editor

-- Tables (using text columns to avoid enum dependency)
...
create policy "forms_select" on public.forms for select using (auth.uid() is not null);
drop policy if exists "forms_all" on public.forms;
create policy "forms_all" on public.forms for all using (auth.uid() is not null);
...
drop policy if exists "form_sections_select" on public.form_sections;
create policy "form_sections_select" on public.form_sections for select using (auth.uid() is not null);
drop policy if exists "form_sections_all" on public.form_sections;
create policy "form_sections_all" on public.form_sections for all using (auth.uid() is not null);
...
create policy "form_fields_select" on public.form_fields for select using (auth.uid() is not null);
drop policy if exists "form_fields_all" on public.form_fields;
create policy "form_fields_all" on public.form_fields for all using (auth.uid() is not null);

create table if not exists public.form_tables (
  id uuid primary key default gen_random_uuid(),
  section_id uuid not null references public.form_sections(id) on delete cascade,
  status_type text not null default 'yes_no',
  has_subrows boolean not null default false,
  order_index int not null default 0
);
alter table public.form_tables enable row level security;
drop policy if exists "form_tables_select" on public.form_tables;
create policy "form_tables_select" on public.form_tables for select using (auth.uid() is not null);
drop policy if exists "form_tables_all" on public.form_tables;
create policy "form_tables_all" on public.form_tables for all using (auth.uid() is not null);
...
drop policy if exists "form_table_rows_select" on public.form_table_rows;
create policy "form_table_rows_select" on public.form_table_rows for select using (auth.uid() is not null);
drop policy if exists "form_table_rows_all" on public.form_table_rows;
create policy "form_table_rows_all" on public.form_table_rows for all using (auth.uid() is not null);

-- Seed NIFPS Installation - Assessment
do $$
declare
  v_form_id uuid;
  v_sec_id  uuid;
begin
  ...
  values ('NIFPS Installation - Assessment', 'site_inspection', 'active', 88)
  ...
  insert into public.form_fields (section_id, label, field_type, is_required, prefill_from_job, read_only_on_mobile, order_index) values
    ...
    (v_sec_id, 'M16 x 80 Bolt, Nut with Washer (Set)', 'number', false, false, false, 7),
    ...
    (v_sec_id, 'M12 x 60 Bolt, Nut with Washer (Set)', 'number', false, false, false, 17);
  ...
  insert into public.form_fields (section_id, label, field_type, is_required, prefill_from_job, read_only_on_mobile, order_index) values
    ...
    (v_sec_id, 'M16 x 60 Fasteners Set',  'number', false, false, false, 3),
  ...
  insert into public.form_fields (section_id, label, field_type, is_required, prefill_from_job, read_only_on_mobile, order_index) values
    ...
    (v_sec_id, 'M16 x 60 Fasteners Set (Standard Pack)', 'number', false, false, false, 15),
    (v_sec_id, 'M12 x 60 Fasteners Set (Standard Pack)', 'number', false, false, false, 16);
  ...
  insert into public.form_fields (section_id, label, field_type, is_required, prefill_from_job, read_only_on_mobile, order_index) values
    (v_sec_id, 'Air Releaser with Reducer for 3 inch Oil Drain Pipe Line', 'number', false, false, false, 1),
    (v_sec_id, 'Air Releaser with Reducer for N2 Pipe Line (1 inch Pipe)', 'number', false, false, false, 2);
