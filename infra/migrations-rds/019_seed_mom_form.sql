-- Seed fresh MOM (Minutes of Meeting) form for Commissioning Activities.
-- Guarded: skips if a form named 'MOM' already exists.
do $$
declare
  v_form_id  uuid;
  v_sec1_id  uuid;
...
begin
  ...
  if exists (select 1 from public.forms where name = 'MOM') then
    return;
  end if;
  ...
  values
    (v_sec1_id, 'Customer Name',        'text',      true,  true,  false, 1),
    (v_sec1_id, 'Contact Number',       'text',      true,  true,  false, 2),
    (v_sec1_id, 'Installation Location','text',      true,  true,  false, 3),
    (v_sec1_id, 'Project Details',      'long_text', false, false, false, 4);
  ...
  insert into public.form_fields
    (section_id, label, field_type, is_required, prefill_from_job, read_only_on_mobile, order_index)
  values
    (v_sec2_id, 'NIFPS Serial No.',     'text', false, true,  false, 1),
    (v_sec2_id, 'Rating',               'text', false, true,  false, 2),
    (v_sec2_id, 'Manufacturer',         'text', false, true,  false, 3),
    (v_sec2_id, 'Site Address',         'text', false, true,  false, 4),
    (v_sec2_id, 'Date of Installation', 'date', true,  false, false, 5),
    (v_sec2_id, 'Duration',             'text', true,  false, false, 6);
  ...
  values (v_sec3_id, 'yes_no', false, 1)
  returning id into v_table3_id;

  insert into public.form_table_rows (table_id, row_label, sno_label, order_index)
  values
  ...
  insert into public.form_tables (section_id, status_type, has_subrows, order_index)
  values (v_sec4_id, 'tested_not_tested', true, 1)
  returning id into v_table4_id;

  -- Row 1 with sub-rows
  insert into public.form_table_rows (table_id, row_label, sno_label, order_index)
  values (v_table4_id, 'Earthing of the below switchyard equipment is to be verified', '1', 1)
  returning id into v_row1_id;

  insert into public.form_table_rows (table_id, parent_row_id, row_label, sno_label, order_index)
  values
  ...
  -- Row 2 with sub-rows
  insert into public.form_table_rows (table_id, row_label, sno_label, order_index)
  values (v_table4_id, 'Potential-free contact assigned with the main protection relay was configured', '2', 5)
  ...
  returning id into v_sec5_id;

  insert into public.form_fields
    (section_id, label, field_type, is_required, prefill_from_job, read_only_on_mobile, order_index)
  values
    (v_sec5_id, 'Customer Name',      'text',      true,  false, false, 1),
    (v_sec5_id, 'Designation',        'text',      false, false, false, 2),
    (v_sec5_id, 'Digital Signature',  'signature', true,  false, false, 3);
