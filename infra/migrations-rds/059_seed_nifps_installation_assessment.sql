-- Seed the "NIFPS Installation - Assessment" form, transcribed from a bilingual
-- (English/Hindi) paper form. Created as draft with a placeholder job_type
-- (arbitrary, unused elsewhere) since it isn't being activated yet — the
-- active-form-per-job_type conflict check only applies to status='active'
-- (see app/actions/save-form.ts), so this is safe to leave as-is until an
-- admin picks the real job_type from the Forms page and activates it.
-- Every section title / field label / table column label / row label carries
-- both its English text and a `_hi` Hindi counterpart (see migration 058) so
-- the mobile fill screen's English/Hindi toggle has real content to show.
do $$
declare
  v_form_id  uuid;
  v_sec_id   uuid;
...
begin
  ...
  values
    ...
    (v_sec_id, 'Phone No.',     'फोन नंबर',       'text', false, false, false, 3),
  ...
  values (v_sec_id, 'measurement', false, 1, 'Description', 'विवरण', 'mm', 'mm')
  ...
  insert into public.form_tables (section_id, status_type, has_subrows, order_index, col1_label, col1_label_hi, col2_label, col2_label_hi)
  values (v_sec_id, 'measurement', false, 1, 'Description', 'विवरण', 'UOM', 'इकाई')
  returning id into v_table_id;

  insert into public.form_table_rows (table_id, row_label, row_label_hi, sno_label, order_index)
  values
    (v_table_id, '80 NB Pipe (GI) (m)', '80 NB पाइप (GI) (m)', '1', 1),
    ...
    (v_table_id, '80 NB U-Bolt with Nut (Set)', '80 NB यू-बोल्ट नट सहित (सेट)', '6', 6),
    (v_table_id, 'M16 × 80 Bolt, Nut with Washer (Set)', 'M16 × 80 बोल्ट, नट व वॉशर (सेट)', '7', 7),
    ...
    (v_table_id, '25 NB Pipe (GI) (m)', '25 NB पाइप (GI) (m)', '11', 11),
    ...
    (v_table_id, 'M12 × 60 Bolt, Nut with Washer (Set)', 'M12 × 60 बोल्ट, नट व वॉशर (सेट)', '17', 17);
  ...
  values
    ...
    (v_table_id, '50 NB Pipe (m)', '50 NB पाइप (m)', '2', 2),
    (v_table_id, 'M16 × 60 Fasteners Set (Set)', 'M16 × 60 फास्टनर्स सेट (सेट)', '3', 3),
  ...
  values
    ...
    (v_table_id, '50 NB Pipe (m)', '50 NB पाइप (m)', '4', 4),
    ...
    (v_table_id, '80 NB Pipe (m)', '80 NB पाइप (m)', '8', 8),
    ...
    (v_table_id, '25 NB Pipe (m)', '25 NB पाइप (m)', '12', 12),
    ...
    (v_table_id, 'M16 × 60 Fasteners Set (Standard Pack) (Set)', 'M16 × 60 फास्टनर्स सेट (स्टैंडर्ड पैक) (सेट)', '15', 15),
    (v_table_id, 'M12 × 60 Fasteners Set (Standard Pack) (Set)', 'M12 × 60 फास्टनर्स सेट (स्टैंडर्ड पैक) (सेट)', '16', 16);
  ...
  insert into public.form_table_rows (table_id, row_label, row_label_hi, sno_label, order_index)
  values
    (v_table_id, '19 Core Cable 1.5 Sq. mm (m)', '19 कोर केबल 1.5 वर्ग मि.मी. (m)', '1', 1),
    (v_table_id, 'LHD Cable (m)', 'LHD केबल (m)', '2', 2),
    (v_table_id, '3 Core Cable 1.5 Sq. mm (m)', '3 कोर केबल 1.5 वर्ग मि.मी. (m)', '3', 3),
    (v_table_id, '2 Core Cable 1.5 Sq. mm (FS) (m)', '2 कोर केबल 1.5 वर्ग मि.मी. (FS) (m)', '4', 4);
  ...
  values
    ...
    (v_sec_id, 'Customer Phone No.',  'ग्राहक फोन नंबर',     'text',      false, false, false, 3),
    (v_sec_id, 'EMR Engineer Name',   'EMR इंजीनियर का नाम', 'text',      true,  true,  false, 4),
    (v_sec_id, 'EMR Signature',       'EMR हस्ताक्षर',       'signature', true,  false, false, 5),
    (v_sec_id, 'EMR Phone No.',       'EMR फोन नंबर',        'text',      false, false, false, 6);
