...
export async function duplicateForm(formId: string, newName: string): Promise<{ error: string | null; newFormId?: string }> {
  // Fetch source form
  const { data: form, error: formErr } = await adminClient().from('forms').select('*').eq('id', formId).single()
  if (formErr || !form) return { error: 'Form not found.' }

  // Create new form as draft
  const { data: newForm, error: newFormErr } = await adminClient().from('forms').insert({
    name: newName,
  ...
  // Copy sections
  const { data: sections } = await adminClient().from('form_sections').select('*').eq('form_id', formId).order('order_index')
  for (const sec of sections || []) {
    const { data: newSec } = await adminClient().from('form_sections').insert({
      form_id: newForm.id, title: sec.title, order_index: sec.order_index,
    }).select().single()
    if (!newSec) continue

    // Copy fields
    const { data: fields } = await adminClient().from('form_fields').select('*').eq('section_id', sec.id).order('order_index')
    for (const f of fields || []) {
      await adminClient().from('form_fields').insert({
    ...
    const { data: tables } = await adminClient().from('form_tables').select('*').eq('section_id', sec.id).order('order_index')
    for (const t of tables || []) {
      const { data: newTable } = await adminClient().from('form_tables').insert({
        section_id: newSec.id, status_type: t.status_type,
        has_subrows: t.has_subrows, order_index: t.order_index,
      }).select().single()
      if (!newTable) continue

      const { data: rows } = await adminClient().from('form_table_rows').select('*').eq('table_id', t.id).order('order_index')
      const rowIdMap: Record<string, string> = {}
      for (const r of rows || []) {
        const { data: newRow } = await adminClient().from('form_table_rows').insert({
