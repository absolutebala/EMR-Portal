...
export async function getWorkOrders(customerId?: string, engineerId?: string): Promise<{ workOrders: WorkOrder[]; error: string | null }> {
  try {
    ...
    const custAddrMap: Record<string, string | null> = {}
    const custPhoneMap: Record<string, string | null> = {}
    customers?.forEach((c: { id: string; name: string; address: string | null; phone: string | null }) => {
      custMap[c.id] = c.name
      custAddrMap[c.id] = c.address
      custPhoneMap[c.id] = c.phone
    })
