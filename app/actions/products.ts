...
// Admin-only bulk delete of catalog products. products.id is ON DELETE RESTRICT from
// product_request_items, so a product used in any request can't be removed — those are
// skipped and reported back (mirrors the customers bulk-delete behaviour).
export async function deleteProductsBulk(ids: string[]): Promise<{ error: string | null; deletedIds?: string[]; skipped?: { id: string; count: number }[] }> {
  const user = await getAuthedUser()
  if (!user) return { error: 'Not authenticated' }
  const admin = adminClient()
  ...
  if (actor?.role !== 'Super Admin' && actor?.role !== 'Head of Service') {
    return { error: 'Only Super Admin or Head of Service can delete products.' }
  }
  const deletedIds: string[] = []
  const skipped: { id: string; count: number }[] = []
  for (const id of ids) {
    const { data: refs } = await admin.from('product_request_items').select('id').eq('product_id', id)
    if (refs && refs.length > 0) { skipped.push({ id, count: refs.length }); continue }
    const { error } = await admin.from('products').delete().eq('id', id)
