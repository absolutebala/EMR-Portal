...
export interface MobileWorkOrderDetail {
  ...
  previousVisits: { id: string; wo_number: string; job_type: string; scheduled_date: string | null; status: string }[]
...
export interface MobileFormField {
  ...
  order_index: number
  // Repeatable "points" field: renders as an add/remove list of point entries on
  // mobile; the value is the points joined by newlines. Defaults false.
  repeatable?: boolean
}
