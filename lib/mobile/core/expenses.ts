...
export async function resolveBLEligibility(admin: AdminClient, userId: string, workOrderId: string): Promise<BLEligibility> {
...
export async function getExpenseTypesCore(admin: AdminClient): Promise<{ types: ExpenseType[]; error: string | null }> {
  try {
...
export async function getOrCreateExpenseTypeCore(admin: AdminClient, name: string): Promise<{ type: ExpenseType | null; error: string | null }> {
  try {
...
// Shared with the desktop-only expense views in app/actions/expenses.ts (admin
...
// expenses" list and the desktop views never drift apart on how a row is built.
...
export async function submitExpenseLogCore(admin: AdminClient, userId: string, params: {
  workOrderId: string
  expenseTypeId: string
  expenseDate: string
  amount: number
  ...
  try {
    ...
    if (params.photo) {
      ...
      if (url) {
        ...
        console.error(`submitExpenseLog: receipt photo upload failed or timed out (path: ${path})`)
      }
    }
    ...
    if (inserted?.id) {
      ;(async () => {
        ...
        notifyUsers(admin, targets, {
          ...
          body: `${actorName} submitted an expense for ${wo?.wo_number || 'a notification'}.`,
          entityType: 'expense_log', entityId: inserted.id,
          linkPath: '/expenses',
        }).catch(() => {})
      })().catch(() => {})
